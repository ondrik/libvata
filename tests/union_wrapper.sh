...
case "${OPERATION}" in
  expl)
    ${VATA} -r expl -t -n union "${FILE_LHS}" "${FILE_RHS}"
    RETVAL="$?"
    ;;
  symdown)
    ${VATA} -r bdd-td -t -n union "${FILE_LHS}" "${FILE_RHS}"
    RETVAL="$?"
    ;;
  symup)
    ${VATA} -r bdd-bu -t -n union "${FILE_LHS}" "${FILE_RHS}"
    RETVAL="$?"
    ;;
  timbuk)
    ${TIMBUK} union "${FILE_LHS}" "${FILE_RHS}"
    RETVAL="$?"
    ;;
